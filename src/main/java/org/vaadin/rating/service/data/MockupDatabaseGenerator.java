...
@WebListener
public class MockupDatabaseGenerator implements ServletContextListener {
    ...
    public void generate() throws Exception {
        ...
        for (int i = 0; i < 50; i++) {
            ...
            p.setSpeaker(mockupSpeakers[(int) (mockupSpeakers.length * Math.
                    random())]);
            p.
                    setTopic(mockupTopics[(int) (mockupTopics.length * Math.
                            random())]);
            p.setStartTime(new Date(
                    System.currentTimeMillis() + (int) (10 * 24 * 60 * 60 * 1000 * Math.
                    random())));
            StringBuilder sb = new StringBuilder();
            while (Math.random() < 0.8) {
                sb.append(
                        mockupOverviewPart[(int) (mockupOverviewPart.length * Math.
                        random())] + " '");
            }
    ...
    public void contextInitialized(ServletContextEvent sce) {
        try {
            ...
            Logger.getLogger(MockupDatabaseGenerator.class.getName()).
